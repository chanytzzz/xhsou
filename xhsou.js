 ...
 * 使用方法：
 *   xhsou "K12教育"
 *   xhsou "K12教育" --date 2026-04-21
 *   xhsou "高途" "作业帮" --max 20
 */

import { searchXhsKeywords } from '/Users/chenyt/Desktop/k12-daily-brief/crawler/xhs-puppeteer.js';
...
配置：
  ...
  位置：/Users/chenyt/Desktop/k12-daily-brief/.env
  `);
}
...
searchXhsKeywords(keywords, {
  ...
  .then(results => {
    ...
    results.forEach((item, i) => {
      ...
      console.log(`   作者: ${item.author} | 点赞: ${item.likes} | 评论: ${item.comments}`);
    ...
    // 导出JSON
    const outputFile = `/tmp/xhsou-${Date.now()}.json`;
    const fs = require('fs');
